...
class Cart:
    ...
    def update(self, product_id, quantity):
        if self.user.is_authenticated:
            CartItem.objects.filter(user=self.user, product_id=product_id).update(
                quantity=quantity
            )
        else:
            product_id = str(product_id)
    ...
    def merge_guest_cart(self):
        """Nach Login: Session-Cart → Database übertragen"""
        if not self.user.is_authenticated:
            return  # Nur für eingeloggte User

        for product_id, data in self.session_cart.items():
            try:
                product = Product.objects.get(id=product_id)
                # Nutzt die add-Methode, die automatisch get_or_create macht
                item, created = CartItem.objects.get_or_create(
                ...
                continue  # Produkt existiert nicht mehr, überspringen

        self.clear()  # Session-Cart leeren
    ...
    def __len__(self):
        if self.user.is_authenticated:
            return sum(
                item.quantity for item in CartItem.objects.filter(user=self.user)
            )
